...
$DriverLog = Join-Path $LogDir "gca_mic_xfm_det_seld_${RunStamp}_driver.log"
$StatusFile = Join-Path $LogDir "gca_mic_xfm_det_seld_${RunStamp}_status.txt"
$GpuLog = Join-Path $LogDir "gca_mic_xfm_det_seld_${RunStamp}_nvidia_smi_live.csv"
...
Write-RunLog "GCA MIC+Transformer deterministic SELD rerun starts"
Write-RunLog "driver_log=$DriverLog"
Write-RunLog "gpu_log=$GpuLog"
Write-RunLog "tasks=141,142 seeds=0,1,2,3,4 best_metric=seld tie=earlier deterministic=strict"
...
try {
    ...
    Write-RunLog "GCA MIC+Transformer deterministic SELD rerun complete"
}
catch {
