# FOA-modality convbias supplement (run this on the FAST machine, e.g. RTX 5060).
# Completes the FOA architecture axis under the SECOND injection mechanism so the
# helps->harms ordering can be compared modality-fixed (FOA) across mechanisms:
#   184/185 = FOA + Conformer   (convbias full / no_geom)
#   186/187 = FOA + Transformer (convbias full / no_geom)
# n=3 seeds -> 4 cells x 3 seeds = 12 runs. Each: train (60 ep) + test_only.
# Auto-skips any cell whose *_test.log already contains "F 20" (resumable).
#
# Only FOA features are needed: foa_dev_norm + foa_dev_adpit_label.
#
# >>> EDIT THESE TWO PATHS for the 5060 machine before running <<<
$DcaseDir = "D:\ssl-research\dcase2024_baseline"          # repo location on this machine
$Python   = "D:\ssl-research\venv\Scripts\python.exe"     # python with torch>=2.7 (Blackwell)
