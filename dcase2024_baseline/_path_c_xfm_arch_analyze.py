"""Path C / Tier VII: cross-architecture replication of GCA ablation.

Reads test_logs for:
  - 140 (Transformer-only, no GCA)        ablate_seed{0,1,2}
  - 141 (Transformer-only, GCA full)      ablate_seed{0,1,2}
  - 142 (Transformer-only, GCA no_geom)   ablate_seed{0,1,2}

Compares against Stage 3 baseline (CRNN: 110/111/112) on shared seeds {0,1,2}
and reports paired contrasts. Tests whether the "geometry-prior hurts" finding
generalizes from CRNN to a non-recurrent (pure-attention) backbone.

Outputs:
    D:\\ssl-research\\paper\\path_c_xfm_arch.json
    D:\\ssl-research\\paper\\path_c_xfm_arch.md
"""
...
CRNN_CELLS = [
    {"task": "110", "name": "110_crnn_gca_full",   "seeds": [0, 1, 2, 3, 4]},
    {"task": "111", "name": "111_crnn_gca_nogeom", "seeds": [0, 1, 2, 3, 4]},
    {"task": "112", "name": "112_crnn_no_gca",     "seeds": [0, 1, 2, 3, 4]},
]
XFM_CELLS = [
    {"task": "140", "name": "140_xfm_no_gca",     "seeds": [0, 1, 2]},
    {"task": "141", "name": "141_xfm_gca_full",   "seeds": [0, 1, 2]},
    {"task": "142", "name": "142_xfm_gca_nogeom", "seeds": [0, 1, 2]},
]
ALL_CELLS = CRNN_CELLS + XFM_CELLS
...
def load_metrics(spec, job_pattern: str = "ablate_seed{seed}") -> dict[int, dict]:
    out: dict[int, dict] = {}
    for s in spec["seeds"]:
        log = _resolve_log(f"dcase2024_{spec['task']}_{job_pattern.format(seed=s)}_test.log")
        m = parse_log(str(log))
        if m is not None:
            out[s] = m
    return out
...
def main() -> int:
    ...
    for spec in ALL_CELLS:
        seeds = load_metrics(spec)
        agg = {"n_seeds": len(seeds), "per_seed": seeds}
        for metric in ("F1", "LE", "DE", "RDE", "SELD"):
            vals = np.array([seeds[k][metric] for k in seeds])
            if len(vals):
                agg[metric] = {
    ...
    pairs = [
        # within-architecture: same as Stage 3 but for Transformer
        ("141_xfm_gca_full",   "142_xfm_gca_nogeom", "Xfm: GCA full vs no_geom"),
        ("141_xfm_gca_full",   "140_xfm_no_gca",     "Xfm: GCA full vs no GCA (overall)"),
        ("142_xfm_gca_nogeom", "140_xfm_no_gca",     "Xfm: channel attention alone vs no GCA"),
        # cross-architecture: same arm comparison
        ("141_xfm_gca_full",   "110_crnn_gca_full",   "GCA full: Xfm vs CRNN"),
        ("142_xfm_gca_nogeom", "111_crnn_gca_nogeom", "GCA no_geom: Xfm vs CRNN"),
        ("140_xfm_no_gca",     "112_crnn_no_gca",     "no GCA: Xfm vs CRNN"),
    ]
    contrasts = {}
    ...
    out_json = OUT_PATH / "path_c_xfm_arch.json"
    out_json.write_text(json.dumps(payload, indent=2, default=str), encoding="utf-8")
    print(f"\n[saved] {out_json}")

    L = ["# Path C / cross-architecture replication (Tier VII)",
         "",
    ...
    out_md = OUT_PATH / "path_c_xfm_arch.md"
