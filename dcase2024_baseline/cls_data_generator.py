...
class DataGenerator(object):
    def __init__(self, params, split=1, shuffle=True, per_file=False, is_eval=False):
        ...
        self._params = params  # keep for SSL-research train_data_fraction subsampling
    ...
    def _get_filenames_list_and_feat_label_sizes(self):
        ...
        # SSL-research Path-C addition: subsample TRAIN files only.
        # Activates when params['train_data_fraction'] in (0,1), the call
        # is non-eval, and the split set does not include the dev-test fold (4).
        # Uses a fixed seed so the same fraction always picks the same files,
        # enabling tight paired comparisons (110-vs-112 at 25%/50%).
        try:
            tdf = float(self._params.get('train_data_fraction', 1.0)) if hasattr(self, '_params') else 1.0
        except Exception:
            tdf = 1.0
        if (not self._is_eval) and 0.0 < tdf < 1.0:
            split_set = set(int(s) for s in np.atleast_1d(self._splits).tolist())
            if 4 not in split_set:  # train-side splits only
                full_files = sorted(self._filenames_list)
                rng = np.random.default_rng(42)
                idx = rng.permutation(len(full_files))
                keep = max(1, int(round(tdf * len(full_files))))
                keep_idx = sorted(idx[:keep].tolist())
                self._filenames_list = [full_files[i] for i in keep_idx]
                print(f"\t[train_data_fraction={tdf}] kept {len(self._filenames_list)}/{len(full_files)} train files")
                # Recompute total_frames over the SUBSAMPLED file list, since
                # _nb_total_batches below depends on it. Without this fix the
                # generator over-runs the (smaller) file list -> IndexError.
                total_frames = 0
                for fname in self._filenames_list:
                    arr = np.load(os.path.join(self._feat_dir, fname))
                    total_frames += (arr.shape[0] - (arr.shape[0] % self._feature_seq_len))
