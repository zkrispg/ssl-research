...
$DriverLog = Join-Path $LogDir "gca_conformer_det_seld_${RunStamp}_driver.log"
$StatusFile = Join-Path $LogDir "gca_conformer_det_seld_${RunStamp}_status.txt"
$GpuLog = Join-Path $LogDir "gca_conformer_det_seld_${RunStamp}_nvidia_smi_live.csv"
...
Write-RunLog "GCA Conformer deterministic SELD rerun starts"
Write-RunLog "driver_log=$DriverLog"
Write-RunLog "gpu_log=$GpuLog"
Write-RunLog "tasks=161,162,171,172 seeds=0,1,2,3,4 best_metric=seld tie=earlier deterministic=strict"
...
try {
    foreach ($seed in 0, 1, 2, 3, 4) {
        foreach ($task in "161", "162", "171", "172") {
    ...
    Write-RunLog "GCA Conformer deterministic SELD rerun complete"
}
catch {
