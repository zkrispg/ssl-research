...
the prior from channel-attention keys to an early convolutional feature bias first
kept the FOA Conformer near zero and flipped the FOA Transformer sign in a
GPU-logged rerun; a strict deterministic rerun with validation-SELD checkpoint
selection then moved both FOA contrasts positive. The MIC+Transformer contrast
also reverses relative to GCA. This rules out stronger universal claims such as
``geometry priors always hurt Transformers'' or ``the Conformer is mechanism-
invariantly neutral.'' It also shows why the primary evidence should remain the
controlled GCA factorial grid and STARSS22 replication, not the second injection
mechanism. The design lesson is narrower and more useful: both the temporal
backbone and the injection site must be ablated, because a hand-crafted prior can
change role when it is imposed before convolution rather than inside channel
attention, and checkpoint selection can move the measured directional error.

\subsection{Practical guidance}
Do not add an array-geometry prior by default: it most likely helps a
low-capacity recurrent model and can hurt a global-mixing Transformer, but the
effect also depends on the injection site. When in doubt, ablate both the prior
and the mechanism that carries it with a seed-matched paired test before
shipping.
...
sensitive to reruns in the Transformer cell, so it bounds rather than confirms
