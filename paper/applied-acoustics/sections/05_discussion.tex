...
The temporal backbone's inductive bias, not the input modality alone, determines
\emph{whether} the prior helps direction---and it does so as a \emph{gradient},
not a switch. A recurrent model has limited capacity to recombine channels over
time, so the geometry bias acts as a useful scaffold; a Transformer already
mixes all channels globally, so the same bias acts as a redundant or mismatched
constraint; the convolution--attention Conformer sits between them and
correspondingly shows a near-neutral to weakly helpful effect. The significant
recurrent-versus-Transformer architecture$\times$prior interaction
(Section~\ref{sec:anova}) and the ordered grid-level pattern are both explained
by this single locality axis, with modality mainly modulating effect size.
...
The Conformer matters because it does \emph{not} collapse to this harmful regime,
yet it also does not become a strong positive case; it is better interpreted as a
near-neutral middle point.
...
(Table~\ref{tab:cross}): recurrent cells help, the Conformer stays
near-neutral, and Transformer cells hurt. MIC+CRNN, null in-domain, is mildly
...
\subsection{The injection mechanism bounds the claim}
The \texttt{convbias} check sharpens the interpretation by limiting it. Moving
the prior from channel-attention keys to an early convolutional feature bias does
not deliver a clean mechanism-invariant replication. Instead, the FOA Conformer
stays near-neutral in early runs but shifts under stricter rerun and
checkpoint-selection paths; FOA Transformer changes sign across reruns; and the
MIC+Transformer contrast reverses relative to GCA. This rules out stronger
claims such as ``geometry priors always hurt Transformers'' or ``the Conformer
is mechanism-invariantly neutral.'' The design lesson is narrower and more
useful: both the temporal backbone and the injection site must be ablated,
because a hand-crafted prior can change role when imposed before convolution
rather than inside channel attention, and checkpoint selection can materially
move the measured directional error.

\subsection{Practical guidance}
Do not add an array-geometry prior by default. In this study it is most useful
for a lower-capacity recurrent model, near-neutral in a Conformer, and capable
of harming a pure-attention Transformer; moreover, alternative injection paths
can themselves be unstable. In practice, the right default is not ``use the
prior'' or ``avoid the prior'' but ``ablate the prior with a matched-capacity,
seed-matched test before shipping.''
...
\emph{(iv)~Backbone, mechanism, and determinism}: we study three backbones
spanning the locality spectrum but a single front-end and recipe, with one
primary geometry mechanism and one smaller second-injection check. That check is
sensitive to reruns and checkpoint selection, so it bounds rather than confirms
the primary GCA conclusion. A deterministic training stack and larger seed budget
would be needed to make stronger claims about alternative injection sites.
